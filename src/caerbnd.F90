...
!-----------------------------------------------------------------------
      ...
      ! Open file.
      call handle_ncerr( nf90_open( 'carbon_emissions.nc', NF_NOWRITE, ncid ), &
         'caerbndini: error opening file '//'carbon_emissions.nc' )
      ...
      call handle_ncerr( nf90_inquire_dimension( ncid, recid, len=nrec ), 'caerbndini: ' )

      ! Check that input data is a right resolution.
      call handle_ncerr( nf90_inq_dimid( ncid, 'lon', did ), 'caerbndini: ' )
      call handle_ncerr( nf90_inquire_dimension( ncid, did, len=nlon ), 'caerbndini: ' )
      ...
      start(3) = lotim
      do m = 1, nspec
         call handle_ncerr( nf90_inq_varid( ncid, vnam(m), vid ), &
            'caerbndini: cannot find variable '//vnam(m) )
         call handle_ncerr( nf90_get_var( ncid, vid, caerin(:,:,m,loin), start, count ), &
      ...
      do m = 1, nspec
         call handle_ncerr( nf90_inq_varid( ncid, vnam(m), vid ), &
            'caerbndini: cannot find variable '//vnam(m) )
         call handle_ncerr( nf90_get_var( ncid, vid, caerin(:,:,m,hiin), start, count ), &
            'caerbndini: cannot read data for '//vnam(m) )
      end do
...
!#######################################################################
   ...
   subroutine caerbndget( lat, nd1, x )
      ...
      integer, intent(in) ::&
         ...
         nd1           ! 1st dimension of x array

      real(r8), intent(out) ::&
         x(nd1,nspec)  ! carbon emissions (kg carbon aerosol/m2/s)
