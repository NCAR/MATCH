...
module scyc
   ...
   integer, parameter :: &
      ncomp = SCYC_NBR,    &! number of sulfur cycle components
      idx1 = SCYC_IDX_SRT   ! start constituent index for first sulfur cycle component
...
!##############################################################################
   ...
   subroutine scycini( xprogsul, emis_type, xvolc_emis )
      ...
      if ( progsul ) then
         write(6,*)'Sulfur cycle configuration:'
         if ( geia_emis ) then
            write(6,*)'  Emissions from 1985 GEIA data prepared by M. Barth.'
         else
            write(6,*)'  Emissions from GEIA/SMITH multiyear dataset.'
         end if
         if ( volc_emis ) then
            write(6,*)'  Include non-eruptive volcano emissions.'
         else
            write(6,*)'  No volcano emissions.'
         end if
      else
         write(6,*)'Sulfur cycle disabled.'
      end if
