...
!-----------------------------------------------------------------------
      ...
      ! Open file.
      call handle_ncerr( nf_open( 'sulfur_emissions.nc', NF_NOWRITE, ncid ), &
         routine_name//': error opening file '//'sulfur_emissions.nc' )
...
!#######################################################################
   ...
   subroutine sulbndget( nd1, nd2, lat, semis )
      ...
      real(r8), intent(out) ::&
         semis(nd1,nd2)  ! sulfur emissions
      ...
      do i = 1, plon
         semis(i,1) = so2(i,lat,1)
         semis(i,2) = so4(i,lat,1)
         semis(i,3) = dms(i,lat,1)
         semis(i,4) = so2(i,lat,2)
         semis(i,5) = so4(i,lat,2)
         semis(i,6) = dms(i,lat,2)
      end do
