...
!-----------------------------------------------------------------------
      ...
      ! Open file.
      call handle_ncerr( nf90_open( trim( ncfile ), NF_NOWRITE, ncid ), &
      ...
      call handle_ncerr( nf90_inquire_dimension( ncid, recid, len=nrec ), 'acbndini: ' )
      ...
      do i = 1, nvar
         call handle_ncerr( nf90_inq_varid( ncid, varnam(i), vid ), &
            'acbndini: cannot find variable '//varnam(i) )

         start(4) = lotim
         call handle_ncerr( nf90_get_var( ncid, vid, datin(:,:,:,loin,i), start, count ), &
            'acbndini: cannot read data for '//varnam(i) )

         start(4) = hitim
         call handle_ncerr( nf90_get_var( ncid, vid, datin(:,:,:,hiin,i), start, count ), &
            'acbndini: cannot read data for '//varnam(i) )
      end do
...
!#######################################################################
   ...
   subroutine acbndget( lat, name, ndim1, slice )

      !----------------------------------------------------------------------- 
      ! Purpose: 
      ! Return annual cycle data for the requested latitude and variable.
      !
      ! Author: B. Eaton
      !----------------------------------------------------------------------- 
      ...
      real(r8), intent(out) ::&
         slice(ndim1,plev)    ! latitude slice of data
